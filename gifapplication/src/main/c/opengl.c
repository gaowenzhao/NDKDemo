...
Java_com_zhao_gifapplication_GifUtils_openFile(JNIEnv *env, jclass type, jstring pathName_) {
	const char *pathName = (*env)->GetStringUTFChars(env, pathName_, 0);
	return openFile(env,pathName_);
//	(*env)->ReleaseStringUTFChars(env, pathName_, pathName);
}

JNIEXPORT jlong JNICALL
Java_com_zhao_gifapplication_GifUtils_renderFrame(JNIEnv *env, jclass type, jlong gifInfo, jobject bitmap) {
	return renderFrame(env,gifInfo,bitmap);
}

JNIEXPORT jint JNICALL
Java_com_zhao_gifapplication_GifUtils_getWidth(JNIEnv *env, jclass type, jlong gifInfo) {
	return getWidth(gifInfo);

}

JNIEXPORT jint JNICALL
Java_com_zhao_gifapplication_GifUtils_getHeight(JNIEnv *env, jclass type, jlong gifInfo) {
